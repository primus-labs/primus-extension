...
const SetPwdDialog: React.FC<SetPwdDialogProps> = memo(
  ({ onSubmit, onClose }) => {
    ...
    const formLegal = useMemo(() => {
      const taskStatusArr = Object.values(taskStatusMap);
      let f = false;
      if (taskStatusArr.length > 1) {
        f = Object.values(taskStatusMap).every((i) => !!i);
      }
      return f;
    }, [taskStatusMap]);
    ...
    const initTaskStatus = useCallback(async () => {
      ...
      if (res[eventId]) {
        ...
        if (lastEventObj) {
          setTaskStatusMap({ ...lastEventObj.steps[0].tasks });
        }
      }
    }, []);
    const setSocialTaskStatus = useCallback(
      async (k, v = 1) => {
        const res = await chrome.storage.local.get([eventId]);
        if (res[eventId]) {
          const lastInfo = JSON.parse(res[eventId]);
          if (!lastInfo.steps[0].tasks) {
            lastInfo.steps[0].tasks = {
              x: 0,
              discord: 0,
            };
          }
          lastInfo.steps[0].tasks[k] = v;
          if (lastInfo.steps[0].tasks.x && lastInfo.steps[0].tasks.discord) {
            lastInfo.steps[0].status = 1;
          }
          await chrome.storage.local.set({
            [eventId]: JSON.stringify(lastInfo),
          });
          initTaskStatus();
        }
      },
