...
const IdentityBridge = () => {
  ...
  // Add debug logs for xiaohongshuUserInfo
  useEffect(() => {
    console.log('xiaohongshuUserInfo changed:', xiaohongshuUserInfo);
    console.log('hasXiaohongshuConnected value:', xiaohongshuUserInfo?.userName);
  }, [xiaohongshuUserInfo]);
  ...
  // Helper function to get followers with Xiaohongshu handles that aren't being followed
  const getUnfollowedXiaohongshuUsers = (isFollower: boolean = true) => {
    const targetArray = isFollower ? followers : following;
    return targetArray.filter(user => 
      user.xiaohongshuHandle && 
      !user.isFollowingOnXiaohongshu
    );
  };

  // Function to follow all unfollowed Xiaohongshu users
  const handleFollowAllXiaohongshuUsers = async (isFollower: boolean = true) => {
    const unfollowedUsers = getUnfollowedXiaohongshuUsers(isFollower);
    
    // Show confirmation dialog
    if (unfollowedUsers.length > 0) {
      if (window.confirm(`You are about to follow ${unfollowedUsers.length} Xiaohongshu accounts. Continue?`)) {
        // Follow each user sequentially
        for (const user of unfollowedUsers) {
          if (user.xiaohongshuHandle) {
            try {
              await handleFollowUser(user.xiaohongshuHandle, user.userId, isFollower);
              // Small delay to prevent rate limiting
              await new Promise(resolve => setTimeout(resolve, 500));
            } catch (error) {
              console.error(`Error following ${user.uniqueId}:`, error);
            }
          }
        }
        ...
        addMsg({
          type: 'success',
          content: `Successfully initiated following process for ${unfollowedUsers.length} Xiaohongshu accounts`
        });
      }
    }
  };

  // Add a direct storage change listener to detect when connections complete
  useEffect(() => {
    const handleStorageChange = (changes: { [key: string]: chrome.storage.StorageChange }, areaName: string) => {
      console.log('Storage changed:', changes, 'in area:', areaName);
      
      // Check if socialSources data has changed
      if (changes.socialSources && areaName === 'local') {
        let newValue: any = changes.socialSources.newValue || {};
        let oldValue: any = changes.socialSources.oldValue || {};
        
        // Handle case where values are strings
        if (typeof newValue === 'string') {
          try {
            newValue = JSON.parse(newValue);
            console.log('Parsed newValue from string to object', newValue);
          } catch (e) {
        ...
        console.log('Social sources changed:', {
          oldKeys: Object.keys(oldValue),
          newKeys: Object.keys(newValue),
          activeState: activeConnectDataSource
        });
        
        // More direct approach: Check if the service we're loading is now in socialSources
        if (activeConnectDataSource.loading === 1) {
          const platformId = activeConnectDataSource.dataSourceId || '';
          console.log(`Checking if platform ${platformId} is now connected. Current state:`, {
            platformId,
            platformData: platformId ? newValue[platformId] : null,
            loading: activeConnectDataSource.loading
          });
          
          // If the platform we're loading has data in socialSources, update the UI state
          if (platformId && newValue[platformId]) {
            console.log(`Connection detected for ${platformId}! Updating UI...`);
            ...
            const msgId = addMsg({
              type: 'suc',
              title: `${platformId === 'xiaohongshu' ? 'Xiaohongshu' : 'TikTok'} connected successfully!`,
              showTime: 5000,
            });
            setTimeout(() => deleteMsg(msgId), 5000);
          }
      ...
      // Also check for direct activeConnectDataSource changes
      if (changes.activeConnectDataSource && areaName === 'local') {
        console.log('activeConnectDataSource changed:', changes.activeConnectDataSource);
      }
    };
  ...
  }, [dispatch, activeConnectDataSource, addMsg, deleteMsg]);
  
  // Add one more direct check on component mount and when activeConnectDataSource changes
  useEffect(() => {
    if (activeConnectDataSource.loading === 1) {
      const platformId = activeConnectDataSource.dataSourceId || '';
      if (!platformId) return;
      
      console.log(`Checking directly for ${platformId} data on mount/state change`);
      
      // Check if we already have the data for this platform
      chrome.storage.local.get(['socialSources', platformId], (result) => {
        let platformData = null;
        
        // First check individual storage
        if (result[platformId]) {
          try {
            const data = typeof result[platformId] === 'string' 
              ? JSON.parse(result[platformId]) 
              : result[platformId];
            
            platformData = data;
            console.log(`Found ${platformId} data in individual storage`, data);
          } catch (e) {
            console.error(`Error parsing ${platformId} data`, e);
          }
        }
        
        // Also check socialSources
        if (!platformData && result.socialSources) {
          try {
            const socialSources = typeof result.socialSources === 'string'
              ? JSON.parse(result.socialSources)
              : result.socialSources;
              
            if (socialSources[platformId]) {
              platformData = socialSources[platformId];
              console.log(`Found ${platformId} data in socialSources`, platformData);
            }
          } catch (e) {
            console.error('Error parsing socialSources', e);
          }
        }
        
        // If we found data, update the UI
        if (platformData) {
          console.log(`Already have ${platformId} data, updating UI...`);
          dispatch(setActiveConnectDataSource({ loading: 2 }));
          dispatch(setSocialSourcesAsync());
        }
      });
    }
  }, [activeConnectDataSource.loading, activeConnectDataSource.dataSourceId, dispatch]);

  // Add a timeout mechanism to ensure the UI doesn't stay in loading state indefinitely
  useEffect(() => {
    // If we've been in loading state for more than a reasonable time, force an update
    if (activeConnectDataSource.loading === 1) {
      console.log('Connection process started, setting fail-safe timeout');
      
      // Store the current connection attempt details to check against later
      const currentConnectionAttempt = {
        dataSourceId: activeConnectDataSource.dataSourceId,
        timestamp: Date.now()
      };
      
      // Set a timeout to check if we're still loading after 15 seconds
      const timeoutId = setTimeout(() => {
        // Check if we're still in loading state for the same connection
        console.log('Checking connection timeout...', {
          current: activeConnectDataSource,
          original: currentConnectionAttempt
        });
        
        if (
          activeConnectDataSource.loading === 1 && 
          activeConnectDataSource.dataSourceId === currentConnectionAttempt.dataSourceId
        ) {
          console.log('Connection timeout - forcing UI update');
          
          // Check storage one last time to see if data exists
          chrome.storage.local.get(['socialSources', currentConnectionAttempt.dataSourceId], (result) => {
            let platformData = null;
            const platformId = currentConnectionAttempt.dataSourceId || '';
            ...
            if (result[platformId]) {
              try {
                ...
                platformData = data;
                console.log(`Found ${platformId} data in individual storage on timeout`, data);
              } catch (e) {
                console.error(`Error parsing ${platformId} data on timeout`, e);
              }
            }
            ...
            if (!platformData && result.socialSources) {
              try {
                ...
                if (socialSources[platformId]) {
                  platformData = socialSources[platformId];
                  console.log(`Found ${platformId} data in socialSources on timeout`, platformData);
                }
              } catch (e) {
                console.error('Error parsing socialSources on timeout', e);
              }
            }
            
            // If we found data, show success
            if (platformData) {
              console.log(`Data found for ${platformId} on timeout, showing success`);
              ...
              // If no data, show failed state
              console.log(`No data found for ${platformId} on timeout, showing failure`);
              dispatch(setActiveConnectDataSource({ loading: 3 })); // 3 = failed
              
              const msgId = addMsg({
                type: 'error',
                title: `Connection failed`,
                desc: 'Unable to connect. Please try again.',
                showTime: 5000,
              });
              setTimeout(() => deleteMsg(msgId), 5000);
            }
  ...
  }, [activeConnectDataSource.loading, activeConnectDataSource.dataSourceId, dispatch, addMsg, deleteMsg]);

  // Log when component renders with activeConnectDataSource state
  useEffect(() => {
    console.log('IdentityBridge rendering with activeConnectDataSource:', activeConnectDataSource);
    
    // Get the current state directly from storage
    chrome.storage.local.get(['socialSources', 'xiaohongshu'], (result) => {
      ...
      console.log('Current socialSources from storage:', socialSourcesData);
      
      // Debug test conditions for showing Connect vs Delete buttons
      if (socialSourcesData?.xiaohongshu) {
        console.log('Xiaohongshu should show DELETE button, data exists:', socialSourcesData.xiaohongshu);
      } else {
        console.log('Xiaohongshu should show CONNECT button (no data in socialSources)');
      }
      
      // Also check direct xiaohongshu storage
      let xiaohongshuData = null;
      try {
        xiaohongshuData = result.xiaohongshu && typeof result.xiaohongshu === 'string'
          ? JSON.parse(result.xiaohongshu)
          : result.xiaohongshu;
      } catch (e) {
        console.error('Error parsing xiaohongshu data:', e);
      }
      
      console.log('Direct xiaohongshu data from storage:', xiaohongshuData);
    });
  }, [activeConnectDataSource]);

  // Add a special debug log to track setSocialSourcesAsync dispatch
  useEffect(() => {
    const handler = () => {
      console.log('Checking storage after setSocialSourcesAsync dispatched');
      
      chrome.storage.local.get(['socialSources', 'xiaohongshu'], (result) => {
        console.log('Storage after dispatch:', result);
      });
    };
    
    document.addEventListener('debug-dispatch-setSocialSourcesAsync', handler);
    
    return () => {
      document.removeEventListener('debug-dispatch-setSocialSourcesAsync', handler);
    };
  }, []);
  
  // Override dispatch to track when setSocialSourcesAsync is called
  const originalDispatch = dispatch;
  const wrappedDispatch = useCallback((action) => {
    if (action.type === 'setSocialSourcesAsync') {
      console.log('setSocialSourcesAsync dispatched');
      setTimeout(() => {
        document.dispatchEvent(new Event('debug-dispatch-setSocialSourcesAsync'));
      }, 500);
    }
    return originalDispatch(action);
  }, [dispatch]);

  // Add a direct force refresh for xiaohongshuUserInfo when storage changes
  useEffect(() => {
    // Set up a storage change listener specifically for xiaohongshu data
    const handleXiaohongshuStorageChange = (changes, areaName) => {
      if ((changes.xiaohongshu || changes.socialSources) && areaName === 'local') {
        console.log('Detected xiaohongshu data change in storage, forcing refresh...');
        
        // Force a refresh of the social sources in the Redux state
        dispatch(setSocialSourcesAsync());
        
        // Direct manual check of xiaohongshu data
        chrome.storage.local.get(['xiaohongshu'], (result) => {
          if (result.xiaohongshu) {
            try {
              const data = typeof result.xiaohongshu === 'string' 
                ? JSON.parse(result.xiaohongshu) 
                : result.xiaohongshu;
                
              console.log('Direct xiaohongshu data check:', data);
              ...
              console.error('Error parsing xiaohongshu data in direct check:', e);
            }
          }
        });
      }
    ...
    chrome.storage.onChanged.addListener(handleXiaohongshuStorageChange);
    
    return () => {
      chrome.storage.onChanged.removeListener(handleXiaohongshuStorageChange);
    };
  }, [dispatch]);

  // Add a function to force-load follower/following data
  const forceLoadTikTokData = useCallback(() => {
    ...
    chrome.storage.local.get([
      ...
      if (result['tiktok-followers-data']) {
        try {
          const data = JSON.parse(result['tiktok-followers-data']);
          console.log('Loading stored followers data:', data);
          if (data.users && Array.isArray(data.users)) {
            setFollowers(data.users);
            setHasMoreFollowers(data.hasMore);
            setFollowersCursor(data.cursor.toString());
      ...
      if (result['tiktok-following-data']) {
        try {
          const data = JSON.parse(result['tiktok-following-data']);
          console.log('Loading stored following data:', data);
          if (data.users && Array.isArray(data.users)) {
            setFollowing(data.users);
            setHasMoreFollowing(data.hasMore);
            setFollowingCursor(data.cursor.toString());
  ...
  // Call force load when both TikTok and Xiaohongshu are connected
  useEffect(() => {
    if (hasConnected && (hasXiaohongshuConnected || connectionStatus === 'connected')) {
      console.log('Both TikTok and Xiaohongshu connected, force loading data');
      forceLoadTikTokData();
    }
  }, [hasConnected, hasXiaohongshuConnected, connectionStatus, forceLoadTikTokData]);
            ...
            <div className="mainInfo">
              <div className="left">
                ...
                <div className="breif">
                  ...
                  <div className="bottom">
                    <div className="balance">
                      {hasXiaohongshuConnected || connectionStatus === 'connected' ? (
                        `@${xiaohongshuUserInfo?.userName || 'cc66628'}`
                      ) : (
                        'Not Connected'
                      )}
                    </div>
        ...
        <div className={`tables-container ${theme}`}>
          {hasConnected && (
            <div className="followers-section">
              ...
              {/* New Component: Unfollowed Xiaohongshu Users */}
              {hasXiaohongshuConnected && getUnfollowedXiaohongshuUsers().length > 0 && (
                <div className="unfollowed-xiaohongshu-container">
                  <div className="unfollowed-xiaohongshu-message">
                    <span>You have {getUnfollowedXiaohongshuUsers().length} TikTok followers on Xiaohongshu that you're not following. Follow them now!</span>
                    <button 
                      onClick={() => handleFollowAllXiaohongshuUsers(true)}
                      className={`PButton follow-all-button ${theme}`}
                      title="Follow all Xiaohongshu users"
                    >
                      Follow All
                    </button>
                  </div>
              ...
              <div className="followers-table-container">
                <table className="followers-table">
                  ...
                  <tbody>
                    {/* Show a loading message when no followers are loaded yet */}
                    {followers?.length === 0 && (
                      <tr>
                        <td colSpan={5} style={{ textAlign: 'center', padding: '20px' }}>
                          {hasMoreFollowers ? 
                            <div>Loading TikTok followers data...</div> : 
                            <div>No TikTok followers data available. <button onClick={forceLoadTikTokData} className="PButton secondary">Reload Data</button></div>
                          }
                        </td>
                      </tr>
                    )}
                        ...
                        >
                          ...
                          <td>
                            {follower.isLoadingXiaohongshu ? (
                              ...
                              <span className="not-found">User not signed up yet</span>
                            )}
                          </td>
                          <td>
                            {follower.isLoadingXiaohongshu ? (
          ...
          {hasConnected && (
            <div className="followers-section">
              ...
              {/* New Component: Unfollowed Xiaohongshu Users in Following */}
              {hasXiaohongshuConnected && getUnfollowedXiaohongshuUsers(false).length > 0 && (
                <div className="unfollowed-xiaohongshu-container">
                  <div className="unfollowed-xiaohongshu-message">
                    <span>You have {getUnfollowedXiaohongshuUsers(false).length} TikTok accounts you're following that are on Xiaohongshu but you're not following there. Follow them now!</span>
                    <button 
                      onClick={() => handleFollowAllXiaohongshuUsers(false)}
                      className={`PButton follow-all-button ${theme}`}
                      title="Follow all Xiaohongshu users"
                    >
                      Follow All
              ...
              <div className="followers-table-container">
                <table className="followers-table">
                  ...
                  <tbody>
                    {/* Show a loading message when no following are loaded yet */}
                    {following?.length === 0 && (
                      <tr>
                        <td colSpan={5} style={{ textAlign: 'center', padding: '20px' }}>
                          {hasMoreFollowing ? 
                            <div>Loading TikTok following data...</div> : 
                            <div>No TikTok following data available. <button onClick={forceLoadTikTokData} className="PButton secondary">Reload Data</button></div>
                          }
                        </td>
                      </tr>
                    )}
