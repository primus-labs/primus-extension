...
import { setAttestLoading, setCredentialsAsync } from '@/store/actions';
    ...
    async (res: any) => {
      ...
      if (retcode === '0') {
        ...
        if (
          ...
          if (assetForm?.verificationContent === 'Assets Proof') {
            descItem1 = `Insufficient assets in your ${
              assetForm?.dataSourceId === 'okx' ? 'Trading' : 'Spot'
            } Account.`;
          }
          let descEl = (
            <>
  ...
  const handleSubmitSetDetail = useCallback(
    async (form) => {
      ...
      const activeWebProofTemplate = webProofTypes.find(
        (i) =>
          i.dataSource === assetForm.dataSourceId &&
          i.name === form.verificationContent
      );
      ...
      responses[responsesLen - 1].conditions.value = form.verificationValue;
